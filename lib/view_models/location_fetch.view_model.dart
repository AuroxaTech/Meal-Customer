...
class LocationFetchViewModel extends MyBaseViewModel {
  ...
  handleFetchCurrentLocation() async {
    ...
    if (granted) {
      ...
      CoolAlert.show(
        ...
        title: "Fetching location".tr(),
        text: "Please wait...".tr(),
        barrierDismissible: false,
      );
      if (LocationService.deliveryaddress != null) {
        controller.text = LocationService.deliveryaddress?.description ?? "";
        controller.selection = TextSelection.fromPosition(
          TextPosition(
              offset:
                  LocationService.deliveryaddress?.description?.length ?? 0),
        );
      }
  ...
  handleFetchSearchLocation() async {
    //final granted = await locationPermissionGetter();
