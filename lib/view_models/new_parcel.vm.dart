...
import 'package:mealknight/requests/package.request.dart';
...
import 'package:mealknight/widgets/bottomsheets/parcel_location_picker_option.bottomsheet.dart';
...
class NewParcelViewModel extends PaymentViewModel {
  ...
  changePickupAddress() async {
    ...
    if (!AuthServices.authenticated()) {
      print('LoginPage  ============= openLogin 55555555');
      final result =
  ...
  changeDropOffAddress() async {
    //check that user is logged in to countinue else go to login page
    if (!AuthServices.authenticated()) {
      print('LoginPage  ============= openLogin 55566666666666');
      final result =
          await Navigator.of(viewContext).pushNamed(AppRoutes.loginRoute);
      paymentOptionRequest = PaymentMethodRequest();
      if (result == null || (result is bool && !result)) {
  ...
  changeStopDeliveryAddress(int index) async {
    //check that user is logged in to countinue else go to login page
    if (!AuthServices.authenticated()) {
      print('LoginPage  ============= openLogin 66666666666');
      final result =
          await Navigator.of(viewContext).pushNamed(AppRoutes.loginRoute);
      paymentOptionRequest = PaymentMethodRequest();
      if (result == null || (result is bool && !result)) {
  ...
  manualChangeStopDeliveryAddress(
    ...
    //check that user is logged in to countinue else go to login page
    if (!AuthServices.authenticated()) {
      print('LoginPage  ============= openLogin 777777777');
      final result =
          await Navigator.of(viewContext).pushNamed(AppRoutes.loginRoute);
      paymentOptionRequest = PaymentMethodRequest();
      if (result == null || (result is bool && !result)) {
  ...
  changeDropOffDate() async {
    ...
    if (result != null) {
      ...
      pickupDate =
          Jiffy.parseFromMillisecondsSinceEpoch(result.millisecondsSinceEpoch)
